...
from .parameters import (
    PARAMETERS, BY_NAME, BY_VH, VHCode,
    get_by_name, get_by_vh, parse_vh_code,
)
...
class RackbusClient:
    ...
    def read_all(self, names: Optional[List[str]] = None) -> Dict[str, Value]:
        ...
        if names is None:
            names = [
                'MASS_FLOW', 'TOTALIZER_1', 'VOLUME_FLOW',
                'DENSITY', 'TEMPERATURE',
                'BUS_ADDRESS', 'CALIBR_FACTOR', 'ZERO_POINT',
                'NOMINAL_SIZE', 'SERIAL_NUMBER', 'TAG_NUMBER',
            ]
        out: Dict[str, Value] = {}
        for n in names:
            try:
