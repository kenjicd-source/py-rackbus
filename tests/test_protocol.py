...
from rackbus_client.codec import (
    even_parity_bit, with_parity, char_to_wire, wire_to_char,
    encode_vr_request, split_frames, parse_response_frame,
    decode_data, parse_value,
)
from rackbus_client.crc import compute_request_crc, verify_request_crc
from rackbus_client.parameters import (
    PARAMETERS, get_by_name, get_by_vh, parse_vh_code,
)
...
@pytest.mark.parametrize("byte, expected_parity", [
    (0x00, 0), (0x01, 1),
...
@pytest.mark.parametrize("byte_in, wire_out", [
    (0x33, 0x33),  # '3'
    (0x52, 0xD2),  # 'R'
    (0x2C, 0xAC),  # ','
    (0x0D, 0x8D),  # CR
...
@pytest.mark.parametrize("char, wire", [
...
@pytest.mark.parametrize("wire, char", [
    (0xCF, '0'), (0x4E, '1'), (0x4D, '2'), (0xCC, '3'),
    (0xCA, '5'), (0xC6, '9'), (0xD2, '-'),
])
...
@pytest.mark.parametrize("addr,v,h,expected_hex", [
...
def test_encode_vr_byte_for_byte(addr, v, h, expected_hex):
    ...
    assert encode_vr_request(addr, v, h) == expected, (
...
@pytest.mark.parametrize("hex_in, decoded, value", [
    ('CFCFDE',          '00!',     0.0),
    ('CADE',            '5!',      0.5),
...
def test_all_fixtures_decode(fixtures):
    ...
    for fx in fixtures:
        ...
        if isinstance(expected, float):
            assert value is not None and abs(value - expected) < 1e-6, (
